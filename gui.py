...
class ObjectDetectionApp:
    def __init__(self, root):
        self.root = root
        self.root.title("A YOLO-based Model for Precise Corrosion Segmentation in Industrial Imagery")
        self.root.geometry("800x800")  # Set window size

        style = ThemedStyle(root)
        style.set_theme("equilux")  # Set the theme for the whole app

        title_label = tk.Label(root, text="A YOLO-based  Corrosion Segmentation ", font=("Helvetica", 20))
